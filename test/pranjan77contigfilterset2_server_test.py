...
class pranjan77contigfilterset2Test(unittest.TestCase):

    @classmethod
    def setUpClass(cls):
        token = environ.get('KB_AUTH_TOKEN', None)
        cls.ctx = {'token': token, 'provenance': [{'service': 'pranjan77contigfilterset2',
        ...
        for nameval in config.items('pranjan77contigfilterset2'):
            cls.cfg[nameval[0]] = nameval[1]
        cls.wsURL = cls.cfg['workspace-url']
        cls.wsClient = workspaceService(cls.wsURL, token=token)
        cls.serviceImpl = pranjan77contigfilterset2(cls.cfg)
    ...
    def getWsName(self):
        ...
        wsName = "test_pranjan77contigfilterset2_" + str(suffix)
